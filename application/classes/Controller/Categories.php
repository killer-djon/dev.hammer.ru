...
class Controller_Categories extends Controller_Main
{
    ...
    public function renderParts($categoryName, $partsName)
    {
        ...
        if ($product->offsetSize() > 0) {
            ...
            $article_keywords = array_combine(array_column($productsArr, 'article'),
                array_column($productsArr, 'name'));
    ...
    public function searchCategory($name = null)
    {
        ...
        if ($category->offsetSize() > 0) {
            ...
            $this->template->content->category_view->parts = [];
        }
    ...
    public function render_category($type = 'view', $name = null)
    {
        ...
        if ($type == 'view') {
            ...
            $this->template->meta_description = 'Интернет-магазин автозапчастей и деталей ФКТ-Автомотив предлагает приобрести детали всех известных производителей ' . implode(', ',
                    array_column($offsets, 'name'));
            ...
            if ($type == 'generic') {
                ...
                $this->template->meta_keywords = implode(',',
                    array_merge($meta_keywords, array_column($offsets, 'name')));
                $this->template->meta_description = 'Производитель автозапчастей и деталей ' . $current['name'] . ' представляет полный набор моделей, от ' . current(array_column($offsets,
                        'name')) . ' до ' . current(array_reverse(array_column($offsets, 'name'), 1));
            } else {
                if ($type == 'engine') {
                    Breadcrumbs::set([
                        URL::base()                                                       => 'Главная',
                    ...
                    $this->template->meta_keywords = implode(',',
                        array_merge($meta_keywords, array_column($offsets, 'name')));
                    $this->template->meta_description = 'Производитель автозапчастей и деталей ' . $current['parentName'] . ' предлагает набор кодов двигателя ' . implode(', ',
                            array_column($offsets, 'name')) . ' модели ' . $current['name'];
                }
            }
        }
