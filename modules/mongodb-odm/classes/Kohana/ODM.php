...
class Kohana_ODM extends Model {
	...
	/**
	 * @var string (key field to disticnt query)
	 */
	protected $_key = '';
	...
	{
		$this->_key = ( !empty( $key ) ? $key : $this->_key );
		$result = ODM::factory($this->_object_name)->load($this->_distinct());
		
		$collection = new ODM_Collection($result);
		return $collection;
	...
	public function lastDocument()
	{
		if( $this->_document )
		{
			return Arr::objectToArray($this->_document);
		}
	 ...
	 * @param bool - load width saves ID or not (true/false)
	 * @return $this
	 */
	public function load($document, $withId = FALSE)
	{
		if ($document)
		{
			...
			if( !$withId )
			{
				if( !Arr::is_assoc($this->_document) )
				{
					array_walk($this->_document, function(&$item, $key){
						if( isset($item['_id']) && !$item['_id'] instanceof MongoId )
						{
							$item['_id'] = new MongoId($item['_id']['$id']);
						}
					});
				}else
				{
					if( isset($this->_document['_id']) && Arr::is_array($this->_document['_id']) )
					{
						$this->_document['_id'] = new MongoId($this->_document['_id']['$id']);
					}
				}	
			}
	...
	/**
	 * Find distinct data from the database
	 *
	 * @throws Exception if not loaded
	 * @return Iterator array
	 */
	protected function _distinct()
	{
		if ($this->_loaded)
		{
			throw new Kohana_Exception('Method distinct() cannot be called on loaded objects');
		}
		
		if( empty($this->_key) )
		{
			throw new Kohana_Exception('Empty fieldkey distinct()');
		}
		
		
		$cursor = $this->_profile(function()
		{
			if( is_array($this->_query) && count($this->_query) )
			{
				return $this->_db->{$this->_collection_name}->distinct($this->_key, $this->_query);
			}
			
			return $this->_db->{$this->_collection_name}->distinct($this->_key);
				
		}, 'distinct', $this->_query);
	...
	protected function _check_type(&$value, $type, $field)
	{
		...
		// Enforce double
		if ($type == 'double' AND (is_string($value) OR is_double($value)))
		{
